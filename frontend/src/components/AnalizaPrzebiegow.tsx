                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      {activeCfg.diagnosis_type === 'Statystyka' ? (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: '10px' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', ...paramStyle, padding: '4px 8px' }}><span>Deadband A:</span>{isSimulating ? <input type="number" step="0.01" value={activeCfg.a_deadband_threshold ?? 0.05} onChange={e => updateOverride('a_deadband_threshold', parseFloat(e.target.value))} style={{...inputStyle, width: '100%'}} /> : <strong style={{marginTop:'4px'}}>{activeCfg.a_deadband_threshold}</strong>}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', ...paramStyle, padding: '4px 8px' }}><span>Deadband Cur:</span>{isSimulating ? <input type="number" step="0.01" value={activeCfg.cur_deadband_threshold ?? 0.05} onChange={e => updateOverride('cur_deadband_threshold', parseFloat(e.target.value))} style={{...inputStyle, width: '100%'}} /> : <strong style={{marginTop:'4px'}}>{activeCfg.cur_deadband_threshold}</strong>}</div>
                        </div>
                      )}
                    </div>
                  );
